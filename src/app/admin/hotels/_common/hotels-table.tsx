...
import { DeleteHotel } from "@/server-actions/hotel";
...
const HotelsTable = ({ hotels }: { hotels: HotelType[] }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: HotelType) => {
        return dayjs(record.createdAt).format("MMM DD, YYYY hh:mm A");
      },
    },
    {
      title: "Action",
  ...
  return (
    <div className="table py-2">
      <Table dataSource={hotels} columns={columns} loading={loading} />
    </div>
  );
};
